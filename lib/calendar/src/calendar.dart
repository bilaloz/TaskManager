...
class TableCalendar extends StatefulWidget {
  ...
  TableCalendar({
    ...
    this.availableCalendarFormats = const {
      CalendarFormat.month: Icon(Icons.keyboard_arrow_down),
      CalendarFormat.week: Icon(Icons.keyboard_arrow_down),
    },
...
class _TableCalendarState extends State<TableCalendar>
  ...
  Widget _buildHeader() {
    ...
    if (widget.headerStyle.formatButtonVisible &&
      ...
      /**
       * TODO : EĞER VISIBLE YAPILMAK ISTENIRSE KKONTROLU BURADA
       */
    }
    ...
    return GestureDetector(
      ...
      child: Padding(
        ...
        child: Container(
          ...
          child: widget.calendarController._getFormatButtonText(),
        ),
      ),
    );
  }
  ...
  Widget _buildMarker(DateTime date, dynamic event) {
    ...
    if (!isCalculateToday(widget.selectedDate, date))
      colors = Colors.green;
    else if (isCalculateToday(dateTime, date)) {
      var isEvent = widget.events.keys.firstWhere(
          (element) => isCalculateToday(element, DateTime.now()),
          orElse: () => null);
      if (isEvent == null)
  ...
  bool isCalculateToday(DateTime one, DateTime two) {
    return ((one.year - two.year == 0) &&
        (one.month - two.month == 0) &&
        (one.day - two.day == 0));
  }

  bool isToday() {
    return (widget.selectedDate.day - DateTime.now().day == 0 &&
        widget.selectedDate.year - DateTime.now().year == 0 &&
        widget.selectedDate.month - DateTime.now().month == 0);
  }

  bool isYesterday() {

    return (widget.selectedDate.day - DateTime.now().day == -1 &&
        widget.selectedDate.year - DateTime.now().year == 0 &&
        widget.selectedDate.month - DateTime.now().month == 0);
  }
    ...
    return Text(
      isToday()
          ...
          : isTomorrow()
              ...
              : isYesterday()
                  ? "Yesterday"
                  : "${DateFormat.EEEE(widget.locale).format(widget.selectedDate)}",
      style: TextStyle(
